...
scp $SSH_OPTS scripts/{remote_setup.sh,remote_runner.sh} $SSH_USER@$SERVER_IP:
ssh $SSH_OPTS $SSH_USER@$SERVER_IP "chmod +x remote_runner.sh remote_setup.sh"

scp $SSH_OPTS scripts/{remote_runner,remote_setup,configure_network,benchmark_nginx}.sh $SSH_USER@$CLIENT_IP:
ssh $SSH_OPTS $SSH_USER@$CLIENT_IP "chmod +x remote_runner.sh remote_setup.sh configure_network.sh benchmark_nginx.sh"
...
SCENARIOS=(
    "cubic 0 0 results/cubic/standard"
    "cubic 20 0 results/cubic/delay_20ms"
    "cubic 20 0.005 results/cubic/delay_20ms_loss_0.005"
    "cubic 20 0.01 results/cubic/delay_20ms_loss_0.01"
    "bbr 0 0 results/bbr/standard"
    "bbr 20 0 results/bbr/delay_20ms"
...
./scripts/plot_aggregate_metrics.py results/
